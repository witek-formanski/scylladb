...
std::string db::snapshot::sstables_location(std::string_view prefix, const replica::table& t, std::string_view snapshot_name) {
    return format_snapshot_location(prefix, sstables::sstables_prefix, t);
}
...
db::snapshot::backup_sstables(db::snapshot_ctl& snap, table_id table_id, std::string tag, std::string endpoint, std::string bucket, std::string prefix, dht::token first_token, dht::token last_token, utils::chunked_vector<sstables::sstable_id> sstable_ids, bool use_move) {
    ...
    if (auto* os = std::get_if<data_dictionary::storage_options::object_storage>(&storage_options.value)) {
        auto table_prefix = os->location.value_or(sstables::sstables_prefix);
        // The sstables of such a table already are in the destination, under the
        // very prefix a backup uploads to, and the snapshot references hold them
        // there for as long as the snapshot lives. There is nothing left to do
        // but record the snapshot as backed up.
