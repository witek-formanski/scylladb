...
- Backup: a cluster backup of a snapshot into the location the keyspace already uses adds no object and no reference of its own. It promotes the snapshot to a backup by recording its SSTables as backed up in `system_distributed.snapshot_sstables`, and writes only its `snapshots/{tag}/manifest.json`. The destination has to be that location, because the component objects are there and a backup addresses them by the same names, and the references a snapshot takes (`sstables/{sstable_id}/refs/snapshot-{tag}/{generation}`) are what keeps them there. A backup to another endpoint, bucket or prefix is rejected, since it would have to copy the objects, and so is a backup which moves the files, since those files are the live data of the table.
