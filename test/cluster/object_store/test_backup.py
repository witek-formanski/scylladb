...
async def test_restore_tablets_dropping_destination_keeps_backup(build_mode: str, manager: ManagerClient, object_storage):
    ...
    therefore delete the objects of the restored copies only, and leave the
    backup the copies were made from untouched, even when the backup and the
    restored keyspace use the same bucket.'''
